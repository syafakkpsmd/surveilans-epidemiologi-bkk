...
export default function FormInputIspaHarian({ onBerhasilSimpan }: { onBerhasilSimpan?: () => void }) {
  ...
  const [ispuStatus, setIspuStatus] = useState<string>('');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setPesanSukses('Data kasus harian berhasil disimpan.');
      resetForm();
      onBerhasilSimpan?.();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm space-y-4">
      <h3 className="text-base font-semibold text-gray-900">Input Kasus ISPA & Kualitas Udara Harian</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="number" min={0} value={kasusAnak}
            onChange={(e) => setKasusAnak(e.target.value)}
            placeholder="0"
            className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          />
        </div>
        ...
        <div>
          ...
          <input
            type="number" min={0} value={kasusDewasa}
            onChange={(e) => setKasusDewasa(e.target.value)}
            placeholder="0"
            className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          />
        </div>
      ...
      >
        {menyimpan ? 'Menyimpan...' : '[+] Simpan Data Kasus Harian'}
      </button>
    </form>
  );
}