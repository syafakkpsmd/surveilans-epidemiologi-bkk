...
export default function GrafikTotalKotaPesawat({
  ...
  const totalPerKota = Object.values(
    data.reduce<Record<string, { kota: string; jumlah_penerbangan: number; total_penumpang: number }>>(
      (acc, row) => {
        if (!acc[row.kota]) acc[row.kota] = { kota: row.kota, jumlah_penerbangan: 0, total_penumpang: 0 };
        acc[row.kota].jumlah_penerbangan += row.jumlah_penerbangan;
        acc[row.kota].total_penumpang += row.total_penumpang;
        return acc;
      },
      {}
    )
